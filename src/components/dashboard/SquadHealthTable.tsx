...
function cellColor(pct: number, isParado: boolean): string {
  if (pct === -1) return "hsl(210 40% 96.1%)";
...
const SquadHealthTable: React.FC<Props> = ({ data }) => {
  ...
  const grouped = new Map<string, SquadHealth[]>();
  data.forEach((row) => {
    const type = row.projectType || "Outros";
    if (!grouped.has(type)) grouped.set(type, []);
    grouped.get(type)!.push(row);
  });

  const typeOrder: string[] = ["Locavia", "Sobrevivência", "Sustentação", "Outros"];
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
          <thead>
            <tr style={{ background: "hsl(210 40% 96.1%)" }}>
              ...
              <th style={{ padding: "8px 8px", textAlign: "left", fontWeight: 600, color: "hsl(215.4 16.3% 46.9%)", fontSize: 10 }}>Tipo</th>
              {COLS.map((c) => (
              ...
              <React.Fragment key={type}>
                <tr>
                  <td colSpan={8} style={{
                    ...
                    background: `${PROJECT_TYPE_COLORS[type as ProjectType] || "hsl(215 16% 47%)"}10`,
                    color: PROJECT_TYPE_COLORS[type as ProjectType] || "hsl(215 16% 47%)",
                    borderTop: "1px solid hsl(214 31.8% 91.4%)",
                  }}>
                    {type}
                  </td>
                </tr>
