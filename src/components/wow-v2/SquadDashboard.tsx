...
const SquadDashboard: React.FC<Props> = ({ rawItems, squadName, sm, accent }) => {
  const { kpis, weeklyData } = useSquadDashboard(rawItems, squadName, sm);
  ...
  return (
    <div style={{
      ...
      {/* Throughput */}
      {sectionLabel("Throughput Semanal", "#2A6B50")}
      <div style={{ background: "#fff", border: "1px solid #e0dcd7", padding: "12px", marginBottom: 4 }}>
        <ResponsiveContainer width="100%" height={200}>
          <ComposedChart data={realData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
            ...
            <Tooltip contentStyle={{ fontSize: 11, background: "#0f1729", color: "#fff", border: "none", borderRadius: 4 }} />
            <Bar yAxisId="left" dataKey="resolvidos" name="Vazão" fill={accent} fillOpacity={0.7} radius={[2, 2, 0, 0]} />
      ...
      {/* Balanço do Fluxo */}
      {sectionLabel("Balanço do Fluxo", "#7B5EA7")}
      <div style={{ background: "#fff", border: "1px solid #e0dcd7", padding: "12px" }}>
        <ResponsiveContainer width="100%" height={200}>
          <BarChart data={realData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
            ...
            <YAxis tick={{ fontSize: 9 }} stroke="#999" />
            <Tooltip contentStyle={{ fontSize: 11, background: "#0f1729", color: "#fff", border: "none", borderRadius: 4 }} />
            <Bar dataKey="criados" name="Demandas Criadas" fill="#7B5EA7" fillOpacity={0.6} radius={[2, 2, 0, 0]} />
            <Bar dataKey="resolvidos" name="Entregas" fill="#2A6B50" fillOpacity={0.7} radius={[2, 2, 0, 0]} />
            <Legend wrapperStyle={{ fontSize: 9 }} />
          </BarChart>
        </ResponsiveContainer>
      </div>
